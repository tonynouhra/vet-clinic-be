"""
Dynamic Pet CRUD Tests - Version-Agnostic API Testing.

Tests pet CRUD operations across all API versions using the dynamic testing framework.
...
class TestPetsDynamic(BaseVersionTest):
    """Dynamic pet CRUD tests across all API versions."""
    ...
    def create_mock_pet(self, api_version: str, test_data_factory: TestDataFactory) -> Pet:
        ...
        if api_version == "v2":
            ...
            pet.additional_photos = ["https://example.com/photo1.jpg"]
    ...
    async def test_create_pet_success(self, api_version: str, async_client: AsyncClient, 
        ...
        with patch("app.pets.controller.PetController.create_pet", new_callable=AsyncMock) as mock_create_pet, \
            ...
            response = await async_client.post(endpoint_url, json=pet_data)
    ...
    async def test_list_pets_with_filters(self, api_version: str, async_client: AsyncClient,
        ...
        with patch("app.pets.controller.PetController.list_pets", new_callable=AsyncMock) as mock_list_pets, \
            ...
            query_params = "?species=dog&gender=male&search=buddy&page=2&per_page=5"
